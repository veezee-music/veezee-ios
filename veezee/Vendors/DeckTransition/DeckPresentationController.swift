...
final class DeckPresentationController: UIPresentationController, UIGestureRecognizerDelegate, DeckSnapshotUpdater {
	...
	private var isTapOutsideDeckToDismissEnabled = true
	...
	private var alphaForPresentingView: CGFloat = 0.8
	private var dimmedOverlayView: UIView?
	...
	convenience init(presentedViewController: UIViewController,
					 ...
					 isTapOutsideDeckToDismissEnabled: Bool,
					 alphaForPresentingView: CGFloat,
		...
		self.isTapOutsideDeckToDismissEnabled = isTapOutsideDeckToDismissEnabled
		self.alphaForPresentingView = alphaForPresentingView
	...
	override var frameOfPresentedViewInContainerView: CGRect {
		...
		let yOffset = ManualLayout.presentingViewTopInset + DeckTransitionConstants.insetForPresentedView
	...
	override func presentationTransitionWillBegin() {
		...
		roundedViewForPresentingView.cornerRadius = presentingViewController.isPresentedWithDeck ? DeckTransitionConstants.cornerRadius : 0
		...
		if presentingViewController.isPresentedWithDeck {
			...
			snapshotView.alpha = alphaForPresentingView
			rootSnapshotView = snapshotView
			
			let snapshotRoundedView = RoundedView()
			snapshotRoundedView.cornerRadius = DeckTransitionConstants.cornerRadius
		...
		presentedViewController.transitionCoordinator?.animate(
			alongsideTransition: { [weak self] context in
				...
				self.roundedViewForPresentingView.cornerRadius = DeckTransitionConstants.cornerRadius
				self.roundedViewForPresentingView.transform = transformForSnapshotView
				self.roundedViewForPresentingView.backgroundColor = UIColor.black.withAlphaComponent(1 - self.alphaForPresentingView)
			}, completion: { _ in
		...
		if isTapOutsideDeckToDismissEnabled {
			dimmedOverlayView = UIView()
			dimmedOverlayView?.frame = snapshotView!.frame
			dimmedOverlayView?.frame.size.height = snapshotView!.frame.height - presentedViewController.preferredContentSize.height
			dimmedOverlayView?.backgroundColor = .clear
			dimmedOverlayView?.isUserInteractionEnabled = true
			if(dimmedOverlayView != nil) {
				containerView.insertSubview(dimmedOverlayView!, aboveSubview: snapshotView!)
			}
			
			let tap = UITapGestureRecognizer(target: self, action: #selector(self.outsideOfDeckTapped));
			dimmedOverlayView?.addGestureRecognizer(tap);
		}
	}
	...
	private func updateSnapshotViewAspectRatio() {
		...
		roundedViewForPresentingView.cornerRadius = DeckTransitionConstants.cornerRadius * scaleForPresentingView
	...
	private func presentedViewWasUpdated() {
		...
		roundedViewForPresentedView.frame = CGRect(x: 0, y: offset, width: containerView!.bounds.width, height: DeckTransitionConstants.cornerRadius)
	}
	...
	override func dismissalTransitionWillBegin() {
		...
		let finalCornerRadius = presentingViewController.isPresentedWithDeck ? DeckTransitionConstants.cornerRadius : 0
		...
		if presentingViewController.isPresentedWithDeck {
			...
			let snapshotRoundedView = RoundedView()
			snapshotRoundedView.cornerRadius = DeckTransitionConstants.cornerRadius
			snapshotRoundedView.backgroundColor = UIColor.black.withAlphaComponent(1 - alphaForPresentingView)
			containerView.insertSubview(snapshotRoundedView, aboveSubview: snapshotView)
			snapshotRoundedView.frame = initialFrame
