...
protocol AudioItemQueueDelegate: AnyObject {
    ...
    /// - Returns: A boolean value indicating whether an item should be consider playable in the queue.
    func audioItemQueue(_ queue: AudioItemQueue, shouldConsiderItem item: PlayableItem) -> Bool
}
...
class AudioItemQueue {
    ...
    private(set) var historic: [PlayableItem]
    ...
    init(items: [PlayableItem], mode: AudioPlayerMode) {
        self.items = items
        self.mode = mode
    ...
    ///
    /// - Returns: The previous item in the queue.
    func previousItem() -> PlayableItem? {
        //Early exit if queue is empty
        guard !queue.isEmpty else {
            return nil
        }
    ...
    ///
    /// - Returns: A boolean value indicating whether an item should be consider playable in the queue.
    private func shouldConsiderItem(item: PlayableItem) -> Bool {
